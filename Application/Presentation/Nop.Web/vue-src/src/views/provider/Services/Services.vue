	...
	v-layout.page-header(row align-center justify-space-between)
		...
		v-flex(shrink)
			v-menu(offset-y left)
				...
				v-list
					v-list-tile(@click="addServiceItem")
				...
				v-layout.service__header(row wrap align-start)
					...
					v-flex.service__info(grow mb-2)
						v-layout(row wrap justify-space-between)
							router-link.service__name( shrink :to="`/provider/services/${service.id}`") {{service.name}}
	...
	methods:{
		addServiceItem() {
			APIServices
				...
				.createServiceItem({})			
				.then(res=>{
					this.serviceItemId = res.data.id;
					this.$router.push(`./services/${this.serviceItemId}/service-info`)
				});
		}
	}
};
