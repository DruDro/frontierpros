<template lang="pug">
	v-layout(align-stretch fluid py-0 )
		v-dialog(v-model="confirmPhonePopup" persistent max-width="700px" attach="#app")
			card.py-5
				v-form(v-model="valid" lazy-validation ref="phoneVerificationForm")
					...
					v-layout.mt-3(align-center justify-center)
						v-flex(shrink)
							v-checkbox(checked v-model="agreed" color="primary" value required :rules="[v => !!v || 'You must agree to continue!']")
								div(slot="label")
